#!/usr/bin/env bash
# Fires at the start of a /refactor skill session via the UserPromptSubmit hook event.
# UserPromptSubmit is a Claude Code built-in lifecycle event that fires on every prompt
# submission, before Claude processes it. Registering here lets us inspect or block a
